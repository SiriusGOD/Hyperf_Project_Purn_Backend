            ...
            case 'coin':
                $query = Coin::join('products', function ($join) {
                            $join->on('coins.id', '=', 'products.correspond_id')
                                ...
                                ->where('expire', Product::EXPIRE['no']);
                        })->selectRaw('products.id, products.name, products.currency, products.selling_price, coins.type')->where('coins.type', Coin::TYPE_LIST[0])->orderBy('coins.points')->get()->toArray();
                break;
            case 'diamond':
                $query = Coin::join('products', function ($join) {
                            $join->on('coins.id', '=', 'products.correspond_id')
                                ->where('products.type', Product::TYPE_LIST[3])
                                ->where('expire', Product::EXPIRE['no']);
                        })->selectRaw('products.id, products.name, products.currency, products.selling_price, coins.type')->where('coins.type', Coin::TYPE_LIST[1])->orderBy('coins.points')->get()->toArray();
                break;
            default:
                $query = MemberLevel::join('products', function ($join) {
                            $join->on('member_levels.id', '=', 'products.correspond_id')
