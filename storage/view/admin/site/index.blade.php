                ...
                <div class="card-body">
                    <div id="example2_wrapper" class="dataTables_wrapper dt-bootstrap4">
                        <div class="row">
                            ...
                            @if(authPermission('site-create'))
                                <div class="col-sm-12 col-md-12 mb-1">
                                    <a class="btn badge-info" href="/admin/site/create">{{trans('default.site_control.site_insert') ?? '新增網站'}}</a>
                                </div>
                            @endif
                        </div>
                        <div class="row">
                                ...
                                <table id="example2" class="table table-bordered table-hover dataTable dtr-inline"
                                    ...
                                    <tr>
                                        ...
                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1"
                                            colspan="1"
                                            aria-label="Browser: activate to sort column ascending">{{trans('default.web_name') ?? '網站名稱'}}
                                        </th>

                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1"
                                            colspan="1"
                                            aria-label="Engine version: activate to sort column ascending">
                                            {{trans('default.web_url') ?? '網址'}}
                                        </th>
                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1"
                                            colspan="1"
                                    ...
                                    @foreach($datas as $model)
                                        <tr class="odd">
                                            ...
                                            <td>{{ $model->url }}</td>
                                            <td>
                                                @if(authPermission('site-edit'))
                                                    <div class="row mb-1">
                                                        <a href="/admin/site/edit?id={{$model->id}}"
                                                           class="btn btn-primary">{{trans('default.edit') ?? '編輯'}}</a>
                                                    </div>
                                                @endif
                                                @if(authPermission('site-delete'))
                                                    <div class="row mb-1">
                                                        <form action="/admin/site/delete" method="post">
