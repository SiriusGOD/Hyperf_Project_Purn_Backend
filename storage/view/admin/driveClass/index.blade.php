                ...
                <div class="card-body">
                    <div id="example2_wrapper" class="dataTables_wrapper dt-bootstrap4">
                        <div class="row">
                            ...
                            @if(authPermission('driveClass-create'))
                                <div class="col-sm-12 col-md-12 mb-1">
                                    <a class="btn badge-info" href="/admin/drive_class/create">{{trans('default.drive_class_control.drive_class_insert') ?? '新增車群類別'}}</a>
                                </div>
                            @endif
                        </div>
                        <div class="row">
                            <div class="col-sm-12">
                                <table id="example2" class="table table-bordered table-hover dataTable dtr-inline"
                                    ...
                                    <tr>
                                        ...
                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1"
                                            colspan="1"
                                            aria-label="Browser: activate to sort column ascending">{{trans('default.drive_class_control.drive_class_name') ?? '车群类别名称'}}
                                        </th>
                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1"
                                            colspan="1"
                                    ...
                                    @foreach($datas as $model)
                                        <tr class="odd">
                                            ...
                                            <td>{{ $model->name }}</td>
                                            <td>
                                            @if(authPermission('driveClass-edit'))
                                                <div class="row mb-1">
                                                <a href="/admin/drive_class/edit?id={{$model->id}}" class="btn btn-primary">{{trans('default.edit') ?? '編輯'}}</a>
                                                </div>
                                            @endif
                                            <!-- @if(authPermission('driveClass-delete'))
                                                <div class="row mb-1">
                                                    <a href="/admin/drive_class/delete?id={{$model->id}}" class="btn btn-danger">{{trans('default.delete') ?? '刪除'}}</a>
                                                </div>
                                            @endif -->
                                            </td>
                                        </tr>
                                    ...
                                    <tr>
                                        ...
                                        <th rowspan="1" colspan="1">{{trans('default.drive_class_control.drive_class_name') ?? '车群类别名称'}}</th>
