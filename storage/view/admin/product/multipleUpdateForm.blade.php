            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div id="example2_wrapper" class="dataTables_wrapper dt-bootstrap4">
                        <div class="row">
                            <form action="/admin/product/multipleUpdateStore" method="post" class="col-md-12" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    ...
                                    <p>{{count(json_decode($product_id_arr, true))}} 筆</p>
                                </div>
                                <div class="form-group">
                                    <label for="exampleInputEmail1">{{trans('default.product_control.product_id') ?? '商品序號'}}</label>
                                    <p>{{$product_id_arr}}</p>
                                </div>
                                <div class="form-group">
                                    <label for="exampleInputEmail1">{{trans('default.product_control.product_type') ?? '商品類型'}}</label>
                                    <p>
                                        {{trans('default.product_control.product_type_msg')[$class_type]}}
                                    </p>
                                </div>
                                <div class="form-group">
                                    <label for="exampleInputEmail1">{{trans('default.type') ?? '類型'}}</label>
                                    <select class="form-control form-control-lg" name="origin_type">
                                        @if($product_type == \App\Model\Product::TYPE_LIST[0])
                                            @foreach(\App\Model\ImageGroup::IMAGE_GROUP_PAY_TYPE as $key => $value)
                                                <option value="{{$value}}" {{($origin->pay_type ?? '') == $value ? 'selected' : ''}}>
                                                    {{trans('default.product_control.product_origin_type')[$value]}}
                                                </option>
                                            @endforeach
                                        @elseif($product_type == \App\Model\Product::TYPE_LIST[1])
                                            @foreach(\App\Model\Video::VIDEO_TYPE as $key => $value)
                                                <option value="{{$value}}" {{($origin->pay_type ?? '') == $value ? 'selected' : ''}}>
                                                    {{trans('default.product_control.product_origin_type')[$value]}}
                                                </option>
                                            @endforeach
